...
class ReferralsCog(commands.Cog):
    ...
    @commands.command(aliases=["refprofit", "rp"])
    async def referral_profit(self, ctx, user: discord.Member = None):
        """View casino profit from a user's current invites (Admin only)
        
        Usage: !referralprofit @user
               !refprofit @user
        """
        # Check if command is being used in the main server
        if ctx.guild.id != self.main_server_id:
        ...
        # Send loading embed
        loading_embed = discord.Embed(
            title="<a:loading:1344611780638412811> | Calculating Profits",
            description="Please wait while we calculate casino profits from invited users.",
            color=0x00FFAE
        )
        loading_message = await ctx.reply(embed=loading_embed)
        ...
        try:
            ...
            # Get all invited user IDs (current invites only)
            invited_users = referral_data.get("invited_users", [])
            
            # Filter out users who have left (not in current invites)
            left_user_ids = set(referral_data.get("left_user_ids", []))
            current_invited_users = [
                user_data for user_data in invited_users 
                if user_data['user_id'] not in left_user_ids
            ]
            
            if not current_invited_users:
                embed = discord.Embed(
                    title="<:no:1344252518305234987> | No Current Invites",
                    description=f"{target_user.mention} has no current invited users.",
                    color=0xFF0000
                )
                return await loading_message.edit(embed=embed)
            
            # Calculate total casino profit from current invited users
            users_db = Users()
            total_wagered = 0
            total_won = 0
            profitable_users = 0
            ...
            for user_data in current_invited_users:
                user_id = user_data['user_id']
                user_account = users_db.fetch_user(user_id)
                
                if user_account:
                    user_wagered = user_account.get("total_spent", 0)
                    user_won = user_account.get("total_earned", 0)
                    ...
                    # Store detailed profit data for top contributors
                    if abs(user_profit) > 100:  # Only include users with significant profit/loss
                        try:
                            discord_user = await self.bot.fetch_user(user_id)
                            username = discord_user.name if discord_user else f"User {user_id}"
                        except:
                            username = f"User {user_id}"
                        
                        detailed_profits.append({
                            "username": username,
                            "user_id": user_id,
                            "profit": user_profit,
            ...
            embed = discord.Embed(
                ...
                description=f"Profit analysis for {target_user.mention}'s current invited users",
                color=0x00FFAE if casino_profit > 0 else 0xFF0000
            )
            ...
            embed.add_field(
                ...
                value=f"**Current Invites:** {len(current_invited_users):,}\n**Total Wagered:** {total_wagered:,.2f} points\n**Total Won:** {total_won:,.2f} points",
                inline=False
            )
            ...
            embed.add_field(
                name=f"{profit_emoji} Casino Profit",
                value=f"**{casino_profit:,.2f}** points\n**${casino_profit_usd:,.2f}** USD equivalent",
                inline=True
            )
            
            # User distribution
            neutral_users = len(current_invited_users) - profitable_users - losing_users
            embed.add_field(
                name="👥 User Distribution",
                value=f"**Profitable:** {profitable_users}\n**Losing:** {losing_users}\n**Neutral:** {neutral_users}",
                inline=True
            )
            
            # Top contributors (if any)
            if detailed_profits:
                detailed_profits.sort(key=lambda x: x["profit"], reverse=True)
                top_contributors = detailed_profits[:5]
                
                contributors_text = ""
                for i, user_detail in enumerate(top_contributors, 1):
                    profit_sign = "+" if user_detail["profit"] >= 0 else ""
                    contributors_text += f"**{i}.** {user_detail['username']}\n"
                    contributors_text += f"Profit: {profit_sign}{user_detail['profit']:,.2f} points\n"
                    contributors_text += f"W: {user_detail['wagered']:,.0f} | L: {user_detail['won']:,.0f}\n\n"
            ...
            if total_wagered > 0:
                profit_rate = (casino_profit / total_wagered) * 100
                embed.add_field(
                    name="📈 Profit Rate",
                    value=f"**{profit_rate:.2f}%** house edge",
                    inline=True
                )
            ...
            embed = discord.Embed(
                title="<:no:1344252518305234987> | Error",
                description=f"An error occurred while calculating profits: {str(e)}",
                color=0xFF0000
            )
            await loading_message.edit(embed=embed)
