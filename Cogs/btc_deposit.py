    ...
    async def check_deposit_button(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        try:
            ...
            if status == "success":
                ...
                # BTC deposits don't add points, only update wallet.BTC
    ...
    async def _check_for_deposits(self, user_id: int, address: str) -> tuple[str, dict]:
        try:
            ...
            for tx in transactions:
                ...
                # BTC deposits only update wallet.BTC - no points conversion
                ...
                print(f"{Fore.GREEN}[+] Processed BTC deposit for user {user_id}: {amount_crypto} BTC (no points conversion), TXID: {txid}{Style.RESET_ALL}")
